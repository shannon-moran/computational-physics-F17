...
class Graph(object):
    ...
    def add_edge_to_matrix(self,p):
        if self.edges[p[0],p[1]]:
            if self.edges[p[1],p[0]]: pass
            else: self.nodes+=1; self.edges[p[1],p[0]] = self.nodes; self.G.add_edge(p[1],p[0])
        else: self.nodes+=1; self.edges[p[0],p[1]] = self.nodes; self.G.add_edge(p[0],p[1])
    ...
    def ODER(self):
        ...
        while self.nodes<self.m:
            ...
            n0 = self.nodes
            self.add_edge_to_matrix(proposed_edge)
            pbar.update(self.nodes-n0)
        pbar.close()
